...
async def main():
    ...
    if use_webhook:
        # Webhook mode: Telegram sends updates to our FastAPI server
        ...
        logging.info(f"LetterCatcher запущен! Webhook + API на порту {API_PORT}")
        ...
        # Polling mode: aiogram polls Telegram + uvicorn serves API
        api_config = uvicorn.Config(
            fastapi_app, host="0.0.0.0", port=API_PORT,
            log_level="info", access_log=False,
        )
        ...
        logging.info(f"LetterCatcher запущен! Polling + API на порту {API_PORT}")
